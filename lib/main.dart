// // import 'package:flutter/material.dart';
// // void main()
// // {
// //   runApp(MyApp());
// // }
// // class MyApp extends StatelessWidget{
// //   const MyApp();
// //   @override
// //   Widget build(BuildContext context){
// //     return MaterialApp(
// //       debugShowCheckedModeBanner:false,
// //       home:Scaffold(
// //         backgroundColor:Colors.cyanAccent,
// //         appBar:AppBar(
// //           leading:CircleAvatar(
// //             child:Icon(Icons.person),),
// //
// //             title:Text(
// //               'welcome',style:TextStyle(backgroundColor:Colors.blue,
// //               color:Colors.white,
// //             ),
// //             ),
// //             centerTitle:true,
// //
// //             actions:
// //               [
// //                 Icon(Icons.search),
// //                 SizedBox(width:40),
// //                 Icon(Icons.more_vert),
// //                 SizedBox(width:30),
// //
// //               ]
// //           ),
// //         body:Center(
// //     child:Column(
// //         children:
// //             [
// //               SizedBox(height:20),
// //            Container(
// //              height:50,
// //              width:400,
// //              child:Center(
// //                child:Text(
// //                  'SIGN UP',style:TextStyle(
// //                  color:Colors.blue,
// //                  fontWeight:FontWeight.bold,
// //                  fontSize:50,
// //                ),
// //                ),
// //              ),
// //            ) ,
// //               SizedBox(height:15),
// //               Container(
// //                 height:50,
// //                 width:400,
// //                 decoration:BoxDecoration(
// //                   color:Colors.blue,
// //                   borderRadius:BorderRadius.circular(10),
// //                 ),
// //                 child:Center(
// //                   child:Text(
// //                     'enter your 1st name',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ),
// //                   ),
// //                 ),
// //               ),
// //               SizedBox(height:15),
// //               Container(
// //                 height:50,
// //                 width:400,
// //                 decoration:BoxDecoration(
// //                   color:Colors.blue,
// //                   borderRadius:BorderRadius.circular(10),
// //                 ),
// //                 child:Center(
// //                   child:Text(
// //                     'enter your last name',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ),
// //                   ),
// //
// //
// //                 ),
// //
// //               ),
// //               SizedBox(height:15),
// //               Container(
// //                 height:50,
// //                 width:400,
// //                 decoration:BoxDecoration(
// //                   color:Colors.blue,
// //                   borderRadius:BorderRadius.circular(10),
// //                 ),
// //                 child:Center(
// //                   child:Text(
// //                    'enter your email',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ) ,
// //                   ),
// //                 ),
// //               ),
// //               SizedBox(height:15),
// //               Container(
// //                 height:50,
// //                 width:400,
// //                 decoration:BoxDecoration(
// //                   color:Colors.blue,
// //                   borderRadius:BorderRadius.circular(10),
// //                 ),
// //                 child:Center(
// //                   child:Text(
// //                     'CNIC',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ),
// //                   ),
// //                 ),
// //               ),
// //               SizedBox(height:15),
// //               Container(
// //                 height:50,
// //                 width:400,
// //                 decoration:BoxDecoration(
// //                   color:Colors.blue,
// //                   borderRadius:BorderRadius.circular(10),
// //                 ),
// //                 child:Center(
// //                   child:Text(
// //                     'enter ur password',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ),
// //                   ),
// //                 ),
// //               ),
// //               SizedBox(height:15),
// //               Container(
// //                 height:50,
// //                 width:400,
// //                 decoration:BoxDecoration(
// //                   color:Colors.blue,
// //                   borderRadius:BorderRadius.circular(10),
// //                 ),
// //                 child:Center(
// //                   child:Text(
// //                     'confirm password',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ),
// //                   ),
// //                 ),
// //               ),
// //               SizedBox(height:15),
// //               Container(
// //                 height:40,
// //                 width:250,
// //                 decoration:BoxDecoration(
// //                   color:Colors.blue,
// //                   borderRadius:BorderRadius.circular(10),
// //                 ),
// //                 child:Center(
// //                   child:Text(
// //                     'Login',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ),
// //                   ),
// //                 ),
// //               ),
// //             ]
// //
// //         ),
// //         ),
// //
// //       ),
// //     );
// //   }
// // }
// // import 'package:flutter/material.dart';
// // void main()
// // {
// //   runApp(MyApp());
// // }
// // class MyApp extends StatelessWidget
// // {
// //   const MyApp();
// //   @override
// //   Widget build(BuildContext context){
// //     return MaterialApp
// //       (
// //       debugShowCheckedModeBanner:false,
// //       home:Scaffold
// //
// //         (
// //         backgroundColor:Colors.black,
// //         appBar:AppBar
// //           (
// //           backgroundColor: Colors.redAccent,
// //           leading:CircleAvatar(
// //             radius:50,
// //             backgroundColor: Colors.pink,
// //             child:Icon(Icons.person)
// //           ),
// //         actions:
// //        [
// //          Icon(Icons.search),
// //          SizedBox(width:30),
// //          Icon(Icons.more_vert),
// //          SizedBox(width:30),
// //       ],
// //         title:Text(
// //           'WANT TO REGISTER??',style:TextStyle(
// //           color:Colors.yellow,
// //           backgroundColor:Colors.black,
// //           fontWeight:FontWeight.bold,
// //         ),
// //         ),
// //         centerTitle:true,
// //         ),
// //
// //       body:Center(
// //
// //
// //       child:Column(
// //          children:
// //       [
// //         SizedBox(height:30),
// //            Container
// //       (
// //           height:100,
// //         width:200,
// //         decoration:BoxDecoration(
// //           color:Colors.pink,
// //           borderRadius:BorderRadius.circular(10),
// //
// //         ),
// //              child:Center(
// //                child:Text(
// //                    'LEARNING FLUTTER',style:TextStyle(
// //                  color:Colors.yellow,
// //                  backgroundColor:Colors.black,
// //                  fontWeight:FontWeight.bold,
// //                ),
// //                ),
// //              ),
// //
// //       ),
// //       SizedBox(height:30,width:100),
// //       CircleAvatar(
// //         radius:80,
// //         backgroundColor:Colors.green,
// //         child:Center(
// //           child:Text(
// //             'EXPLORING',style:TextStyle(
// //             color:Colors.yellow,
// //             backgroundColor:Colors.black,
// //             fontWeight:FontWeight.bold,
// //           ),
// //           )
// //         ),
// //       ),
// //         SizedBox(height:30),
// //         Container(
// //           height:100,
// //           width:200,
// //           decoration:BoxDecoration(
// //             color:Colors.pink,
// //             borderRadius:BorderRadius.circular(10),
// //
// //           ),
// //           child:Center(
// //             child:Text(
// //             'LAIBA UMAIR',style:TextStyle(
// //             color:Colors.yellow,
// //             backgroundColor:Colors.black,
// //             fontWeight:FontWeight.bold,
// //           ),
// //           ),
// //           ),
// //         ),
// //
// //        ],
// //       ),
// //     ),
// //       ),
// //
// //     );
// //   }
// //
// // }
// // import 'package:flutter/material.dart';
// // void main()
// // {
// //   runApp(MyApp());
// // }
// // class MyApp extends StatelessWidget
// // {
// //   const MyApp();
// //
// //   @override
// //   Widget build(BuildContext context){
// //     return MaterialApp(
// //       debugShowCheckedModeBanner:false,
// //      home:Scaffold(
// //        backgroundColor: Colors.pink,
// //        appBar:AppBar(
// //          backgroundColor: Colors.black,
// //          leading:CircleAvatar(
// //            radius:15,
// //            backgroundColor:Colors.yellow,
// //            child:Icon(Icons.person),
// //          ),
// //          actions:
// //            [
// //              Icon(Icons.search),
// //              SizedBox(width:15),
// //              Icon(Icons.more_vert),
// //              SizedBox(width:15),
// //            ],
// //          title:Text(
// //            'laiba umair',style:TextStyle(
// //            color:Colors.yellow,
// //            fontWeight:FontWeight.bold,
// //          ),
// //          ),
// //          centerTitle:true,
// //        ),
// //        body:SingleChildScrollView(
// //          scrollDirection:Axis.horizontal,
// //          child:Row(
// //            children:[
// //           Container(
// //             height:270,
// //             width:160,
// //             decoration:BoxDecoration(
// //               color:Colors.yellow,
// //               borderRadius:BorderRadius.circular(10),
// //             ),
// //             child:Column(
// //               children:[
// //                 SizedBox(height:20),
// //                 CircleAvatar(
// //               radius:30,
// //               backgroundColor:Colors.pink,
// //               child:Text(
// //                 '1'
// //               ) ,
// //                 ),
// //                 SizedBox(height:150),
// //                 Text(
// //                   'heloo i am doing web development',style:TextStyle(
// //                   color:Colors.black,fontWeight:FontWeight.bold,
// //                 ),
// //                 ),
// //               ],
// //             ),
// //           ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:160,
// //                decoration:BoxDecoration(
// //                  color:Colors.orangeAccent,
// //                  borderRadius:BorderRadius.circular(10),
// //                ),
// //                child:Column(
// //                  children:[
// //                    SizedBox(height:20),
// //                   CircleAvatar(
// //                     backgroundColor:Colors.pink,
// //                     radius:30,
// //                   child:Text(
// //                     '2',style:TextStyle(
// //                     color:Colors.white,
// //                     fontWeight:FontWeight.bold,
// //                   ),
// //                   ),
// //                   ),
// //                    SizedBox(height:150),
// //                    Text(
// //                      'learned C++',style:TextStyle(
// //                      color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    )
// //                    ),
// //                  ],
// //                ),
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:160,
// //                decoration:BoxDecoration(
// //                  color:Colors.cyanAccent,
// //                  borderRadius:BorderRadius.circular(10),
// //              ),
// //                child:Column(
// //                  children:[
// //                    SizedBox(height:20),
// //                    CircleAvatar(
// //                      backgroundColor:Colors.pink,
// //                      radius:30,
// //                      child:Text(
// //                        '3',style:TextStyle(
// //                        color:Colors.white,
// //                      ),
// //                      ),
// //
// //                    ),
// //                    SizedBox(height:150),
// //                    Text(
// //                      'learned C-sharp',style:TextStyle(
// //                      color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    ),
// //                    ),
// //                  ],
// //                ),
// //
// //
// //
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:160,
// //                decoration:BoxDecoration(
// //                  color:Colors.deepPurpleAccent,
// //                  borderRadius:BorderRadius.circular(10),
// //                ),
// //                child:Column(
// //
// //                  children:[
// //                  SizedBox(height:20),
// //                  CircleAvatar(
// //                    radius:30
// // ,                 backgroundColor:Colors.pink,
// //                    child:Text(
// //                      '4',style:TextStyle(
// //                      color:Colors.white,
// //                    ),
// //                    ),
// //                  ) ,
// //                    SizedBox(height:150),
// //                    Text(
// //                      'learned C-Sharp windows',style:TextStyle(
// //                      color:Colors.black,fontWeight:FontWeight.bold,
// //                    ),
// //                    ),
// //                  ],
// //                ),
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:160,
// //              decoration:BoxDecoration(
// //                color:Colors.red,
// //                borderRadius:BorderRadius.circular(10),
// //              ),
// //                child:Column(
// //                  children:[
// //                    SizedBox(height:20),
// //                    CircleAvatar(
// //                      radius:30,
// //                      backgroundColor:Colors.pink,
// //                      child:Text(
// //                        '5',style:TextStyle(
// //                        color:Colors.white,
// //                      ),
// //                      ),
// //
// //                    ),
// //                    SizedBox(height:150),
// //                    Text(
// //                      'learned powerpoint',style:TextStyle(
// //                      color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    ),
// //                    ),
// //                  ],
// //                ),
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:160,
// //                decoration:BoxDecoration(
// //                  color:Colors.blue,
// //                  borderRadius:BorderRadius.circular(10),
// //                ),
// //                child:Column(
// //                  children:[
// //                    SizedBox(height:20),
// //                    CircleAvatar(
// //                      radius:30,
// //                      backgroundColor:Colors.pink,
// //                      child:Text(
// //                        '6',style:TextStyle(
// //                        color:Colors.white,
// //                      ),
// //                      ),
// //                    ),
// //                    SizedBox(height:150),
// //                    Text(
// //                      'learned MS excel',style:TextStyle(
// //                      color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    ),
// //                    ),
// //                  ],
// //                ),
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:160,
// //                decoration:BoxDecoration(color:Colors.brown,
// //                  borderRadius:BorderRadius.circular(10),
// //                  ),
// //                  child:Column(
// //                  children:
// //                  [
// //                    SizedBox(height:20),
// //                    CircleAvatar(
// //                  radius:30,
// //                  backgroundColor:Colors.pink,
// //                    child:Text(
// //                    '7',style:TextStyle(
// //                    color:Colors.white,
// //                    fontWeight:FontWeight.bold,
// //                  ),
// //                  ),
// //                    ),
// //                    SizedBox(height:150),
// //                    Text(
// //                      'learning flutter',style:TextStyle(
// //                        color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    ),
// //                    ),
// //                  ],
// //              ),
// //
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:140,
// //                decoration:BoxDecoration(
// //                  color:Colors.deepPurpleAccent,
// //                    borderRadius:BorderRadius.circular(10),
// //                ),
// //                child:Column(
// //                  children:[
// //                    SizedBox(height:20),
// //                    CircleAvatar(
// //                      radius:30,
// //                      backgroundColor:Colors.pink,
// //                      child:Text(
// //                        '8',style:TextStyle(
// //                        color:Colors.white,
// //                      ),
// //                      ),
// //                    ),
// //                    SizedBox(height:150),
// //                    Text(
// //                        'learned syntax of flutter',style:TextStyle(
// //                      color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    ),
// //                    ),
// //                  ],
// //                ),
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:140,
// //                decoration:BoxDecoration(
// //                  color:Colors.amber,
// //                  borderRadius:BorderRadius.circular(10),
// //                ),
// //                child:Column(
// //                  children:[
// //                    SizedBox(height:20),
// //                    CircleAvatar(
// //                      radius:30,
// //                      backgroundColor:Colors.pink,
// //                      child:Text(
// //                        '9',style:TextStyle(
// //                        color:Colors.white,
// //                      ),
// //                      ),
// //                    ),
// //                    SizedBox(height:150),
// //                    Text(
// //                    'learned different widget',style:TextStyle(
// //                  color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    ),
// //              ),
// //                  ],
// //                ),
// //              ),
// //              SizedBox(width:10),
// //              Container(
// //                height:270,
// //                width:140,
// //                decoration:BoxDecoration(
// //                color:Colors.lightGreenAccent,
// //                borderRadius:BorderRadius.circular(10),
// //              ),
// //                child:Column(
// //                  children:[
// //                 SizedBox(height:20),
// //                 CircleAvatar(
// //                   radius:30,
// //                   backgroundColor:Colors.pink,
// //                   child:Text(
// //                     '10',style:TextStyle(
// //                     color:Colors.white,
// //                   ),
// //                   ),
// //                 ) ,
// //                    SizedBox(height:130),
// //                    Text(
// //                      'learnt about multiple properties of different widgets',style:TextStyle(
// //                      color:Colors.black,
// //                      fontWeight:FontWeight.bold,
// //                    ),
// //                    ),
// //                  ],
// //                ),
// //              ),
// //              SizedBox(width:10),
// //          ],
// //            ),
// //
// //          ),
// //        ),
// //
// //     );
// //   }
// // }
// // import 'package:flutter/material.dart';
// //
// // void main() {
// //   runApp(MyApp());
// // }
// //
// // class MyApp extends StatelessWidget {
// //   @override
// //   Widget build(BuildContext context) {
// //     return MaterialApp(
// //       debugShowCheckedModeBanner:false,
// //       home: Scaffold(
// //         backgroundColor: Colors.black,
// //         appBar: AppBar(
// //             backgroundColor: Colors.teal,
// //             leading:CircleAvatar(
// //               radius:20,
// //               backgroundColor: Colors.black,
// //               child:Icon(Icons.person_add_alt_1_rounded),
// //             ),
// //             actions:[
// //               Icon(Icons.search),
// //               SizedBox(width:14),
// //               Icon(Icons.more_vert),
// //               SizedBox(width:14),
// //             ],
// //
// //
// //             title: Text('trying something new'),
// //             centerTitle:true,
// //         ),
// //         body:SingleChildScrollView(
// //           scrollDirection:Axis.horizontal,
// //
// //         child:Column(
// //           children: [
// //             // First Row with multiple containers
// //             Row(
// //               mainAxisAlignment: MainAxisAlignment.spaceEvenly,
// //               children: [
// //                 Container(
// //                   width: 500,
// //                   height: 50,
// //                   color: Colors.red,
// //                 ),
// //                 SizedBox(width:15),
// //                 Container(
// //                   width: 500,
// //                   height: 50,
// //                   color: Colors.blue,
// //                 ),
// //                 SizedBox(width:15),
// //                 Container(
// //                   width: 500,
// //                   height: 50,
// //                   color: Colors.green,
// //                 ),
// //               ],
// //             ),
// //             SizedBox(height: 20), // Spacing between rows
// //             // Second Row with text
// //             Text(
// //               'LAIBA UMAIR APP DEVELOPER',
// //               style: TextStyle(color:Colors.red,fontSize: 50, fontWeight: FontWeight.bold),
// //             ),
// //           ],
// //         ),
// //       ),
// //       ),
// //     );
// //   }
// // // }
// import 'package:flutter/cupertino.dart';
// import 'package:flutter/material.dart';
// void main()
// {
//   runApp(MyApp());
// }
// class MyApp extends StatelessWidget{
//   const MyApp();
//   @override
//   Widget build(BuildContext context){
//     return MaterialApp(
//       debugShowCheckedModeBanner:false,
//       home:Scaffold(
//         backgroundColor:Colors.white,
//         appBar:AppBar(
//           backgroundColor:Colors.white,
//           actions:[
//
//             Icon(Icons.add_circle,
//             color:Colors.black,),
//             SizedBox(width:80),
//             Icon(Icons.search,color:Colors.black,),
//             SizedBox(width:80),
//             Icon(Icons.message,color:Colors.black,),
//             SizedBox(width:80),
//           ],
//           title:Text(
//             'facebook',style:TextStyle(
//             color:Colors.blue,
//             fontWeight:FontWeight.bold,
//             fontSize:40,
//           ),
//           ),
//           centerTitle:false,
//         ),
//         body:SingleChildScrollView(
//         child:Column(
//           children:[
//             Row(
//               mainAxisAlignment:MainAxisAlignment.spaceEvenly,
//               children:[
//                 Icon(Icons.home,size:40,),
//                 Icon(Icons.video_camera_back_outlined,size:40,),
//                 Icon(Icons.person,size:40,),
//                 Icon(Icons.shopping_bag,size:40,),
//                 Icon(Icons.notifications,size:40,),
//                 Icon(Icons.menu,size:40,),
//
//               ],
//             ),
//             Divider(thickness:2,color:Colors.black26),
//             Row(
//               mainAxisAlignment: MainAxisAlignment.spaceEvenly,
//              children:[
//                CircleAvatar(
//                  backgroundColor:Colors.black,
//                  radius:30,
//                  backgroundImage:AssetImage('assets/beutifull.jpg'),
//
//                ),
//                Container(height:30,
//                width:300,
//                decoration:BoxDecoration(
//                  color:Colors.white,
//                  borderRadius:BorderRadius.circular(10),
//                  border:Border.all(
//                    color:Colors.black,width:0.5,
//                  ),
//                ),
//                child:Center(child:Text(
//                  'whats on your mind?',style:TextStyle(
//                  color:Colors.black,fontWeight:FontWeight.bold,
//                ),
//                )
//                ),
//                ),
//                Icon(Icons.photo_size_select_actual_outlined,color:Colors.green,),
//                SizedBox(width:300),
//              ],
//             ),
//             Divider(thickness:2,color:Colors.blueGrey,),
//             SingleChildScrollView(
//               scrollDirection: Axis.horizontal,
//               child: Row
//                 (
//                 children:
//                 [
//                      Column(
//                        children:[
//                                Container(
//                              height:175,
//                                  width:160,
//                                  decoration:BoxDecoration(
//                                    color:Colors.white,
//                                    borderRadius:BorderRadius.circular(10),
//                                    border:Border.all(
//                                      color:Colors.black,
//                                    ),
//                                    image:DecorationImage(
//                                      image:AssetImage('assets/beutifull.jpg'),
//                                      fit:BoxFit.cover,
//                                    ),
//                                  ),
//               ),
//                                  Container(
//                                    height:60,
//                                  width:160,
//                                  decoration:BoxDecoration(
//                                    color:Colors.white,
//                                    borderRadius:BorderRadius.circular(10),
//                                    border:Border.all(
//                                        color:Colors.black,
//                                    ),
//                                  ),
//                                  child:Column(
//                                   crossAxisAlignment:CrossAxisAlignment.center,
//                                    children:[
//                                  Icon(Icons.add_circle,color:Colors.blue,size:25),
//                                  SizedBox(height:10),
//                                  Text(
//                                    'Laiba Umair',style:TextStyle(
//                                    color:Colors.black,
//                                    fontWeight:FontWeight.bold,
//                                  ),
//                                  ),
//     ],
//     ),
//     ),
//                                    ],
//                                  ),
//                          SizedBox(width:10),
//                          Container(
//                            height:240,
//                            width:160,
//                            decoration:BoxDecoration(
//                              color:Colors.purple,
//                              borderRadius:BorderRadius.circular(10),
//                            image:DecorationImage(
//                              image:AssetImage('assets/pictwo.jpg'),
//                              fit:BoxFit.cover,
//                            ),
//                            ),
//
//                            child:Padding(padding:EdgeInsets.all(10.0),
//                                  child:Column(
//                                    crossAxisAlignment:CrossAxisAlignment.start,
//                                    children:[
//                                      CircleAvatar(
//                                        radius:20,
//                                        backgroundColor:Colors.yellow,
//                                        backgroundImage:AssetImage('assets/hijab.jpg'),
//                                      ),
//                                      SizedBox(height:150),
//                                      Text(
//                                        'faiza ahmed',style:TextStyle(
//                                        color:Colors.white,
//                                        fontWeight:FontWeight.bold,
//                                      ),
//                                      ),
//                                    ],
//                                  ),
//                                ),
//                          ),
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       color:Colors.brown,
//                       borderRadius:BorderRadius.circular(10),
//                     image:DecorationImage(
//                       image:AssetImage('assets/awesome.jpg'),
//                       fit:BoxFit.cover,
//                     ),
//                     ),
//                     child:Padding(
//                           padding:EdgeInsets.all(10.0),
//                          child:Column(
//                            crossAxisAlignment:CrossAxisAlignment.start,
//                   children:[
//
//                             CircleAvatar(
//                               radius:20,
//                               backgroundColor:Colors.yellow,
//                             backgroundImage:AssetImage('assets/beutifull.jpg',
//                               ),
//                             ),
//                             SizedBox(height:150),
//                             Text(
//                               'ayesha khan',style:TextStyle(
//                               color:Colors.white,
//                               fontWeight:FontWeight.bold,
//                             ),
//                             ),
//                           ],
//                         ),
//     ),
//                   ),
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       color:Colors.purpleAccent,
//                       borderRadius:BorderRadius.circular(10),
//                       image:DecorationImage(
//                         image:AssetImage('assets/boy.jpg'),
//                         fit:BoxFit.cover,
//                       ),
//                     ),
//                     child: Padding(padding:EdgeInsets.all(10.0),
//                       child:Column(
//                         crossAxisAlignment: CrossAxisAlignment.start,
//                         children:[
//                           CircleAvatar(
//                             radius:20,
//                             backgroundColor:Colors.yellow,
//                              backgroundImage:AssetImage('assets/boy2.jpg'),
//                           ),
//                           SizedBox(height:150),
//                           Text(
//                             'Usman khan',style:TextStyle(
//                             color:Colors.white,
//                             fontWeight:FontWeight.bold,
//                           ),
//                           ),
//                         ],
//                       ),
//                     ),
//                   ),
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       color:Colors.red,
//                       borderRadius:BorderRadius.circular(10),
//                       image:DecorationImage(
//                         image:AssetImage('assets/pic3.jpg'),
//                         fit:BoxFit.cover,
//                       ),
//
//                     ),
//                     child:Padding(padding:EdgeInsets.all(10.0),
//                     child:Column(
//                       crossAxisAlignment: CrossAxisAlignment.start,
//                       children:[
//                     CircleAvatar(
//                       radius:20,
//                       backgroundColor:Colors.yellow,
//                       backgroundImage:AssetImage('beach.jpg'),
//                     ),
//                     SizedBox(height:150),
//                     Text(
//                       'freedom',style:TextStyle(
//                       color:Colors.white,
//                       fontWeight:FontWeight.bold,
//                     ),
//                     ),
//                         ],
//                     ),
//                     ),
//                   ),
//
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       color:Colors.orangeAccent,
//                       borderRadius:BorderRadius.circular(10),
//                       image:DecorationImage(
//                         image:AssetImage('assets/bday.jpg',),
//                         fit:BoxFit.cover,
//                       ),
//                     ),
//                     child:Padding(
//                       padding:EdgeInsets.all(10.0),
//                       child:Column(
//                         crossAxisAlignment:CrossAxisAlignment.start,
//                         children:
//                         [
//                           CircleAvatar(
//                             radius:20,
//                             backgroundColor:Colors.yellow,
//                             backgroundImage:AssetImage('assets/boy5.jpg'),
//                           ),
//                           SizedBox(height:150),
//                           Text(
//                             'sad_person',style:TextStyle(
//                             color:Colors.white,
//                             fontWeight:FontWeight.bold,
//                           ),
//                           ),
//                         ],
//                       ),
//                     ),
//                   ),
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       color:Colors.orangeAccent,
//                       borderRadius:BorderRadius.circular(10),
//                       image:DecorationImage(
//                         image:AssetImage('assets/boy4.jpg',),
//                         fit:BoxFit.cover,
//                       ),
//                     ),
//                     child:Padding(
//                       padding:EdgeInsets.all(10.0),
//                       child:Column(
//                         crossAxisAlignment:CrossAxisAlignment.start,
//                         children:
//                         [
//                           CircleAvatar(
//                             radius:20,
//                             backgroundColor:Colors.yellow,
//                             backgroundImage:AssetImage('assets/boy7.jpg'),
//                           ),
//                           SizedBox(height:150),
//                           Text(
//                             'Ayan khan',style:TextStyle(
//                             color:Colors.white,
//                             fontWeight:FontWeight.bold,
//                           ),
//                           ),
//                         ],
//                       ),
//                     ),
//                   ),
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       borderRadius:BorderRadius.circular(10),
//                       image:DecorationImage(
//                         image:AssetImage('assets/girl8.jpg'),
//                         fit:BoxFit.cover,
//                       ),
//                     ),
//                     child:Padding(
//                           padding:EdgeInsets.all(10.0),
//                           child:Column(
//                             crossAxisAlignment:CrossAxisAlignment.start,
//                             children:[
//                               CircleAvatar(
//                                 radius:20,
//                                 backgroundImage:AssetImage('assets/girl2.jpg'),
//                               ),
//                               SizedBox(height:150),
//                               Text(
//                                 'girl in heaven ',style:TextStyle(
//                                 color:Colors.white,fontWeight:FontWeight.bold,
//                               ),
//                               ),
//                             ],
//                           ),
//                         ),
//                     ),
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       color:Colors.blueGrey,
//                       borderRadius:BorderRadius.circular(10),
//                       image:DecorationImage(
//                         image:AssetImage('assets/girl5.jpg'),
//                         fit:BoxFit.cover,
//                       ),
//                     ),
//                     child:Padding(
//                       padding:EdgeInsets.all(10.0),
//                       child:Column(
//                         crossAxisAlignment:CrossAxisAlignment.start,
//                         children:[
//                           CircleAvatar(
//                             radius:20,
//                             backgroundImage:AssetImage('assets/girl.jpg'),
//                           ),
//                           SizedBox(height:150),
//                           Text(
//                             'Bezubaan',style:TextStyle(
//                             color:Colors.white,
//                             fontWeight:FontWeight.bold,
//                           ),
//                           ),
//                         ],
//                       ),
//                     ),
//                   ),
//                   SizedBox(width:10),
//                   Container(
//                     height:240,
//                     width:160,
//                     decoration:BoxDecoration(
//                       borderRadius:BorderRadius.circular(10),
//                       image:DecorationImage(
//                         image:AssetImage('assets/quote.jpg'),
//                         fit:BoxFit.cover,
//                       ),
//                     ),
//                     child:Padding(
//                       padding:EdgeInsets.all(10.0),
//                       child:Column(
//                         crossAxisAlignment:CrossAxisAlignment.start,
//                         children:[
//                           CircleAvatar(
//                             radius:20,
//                             backgroundImage:AssetImage('assets/quote2.jpg'),
//                           ),
//                           SizedBox(height:150),
//                           Text(
//                             'Acheiver',style:TextStyle(
//                             color:Colors.white,
//                             fontWeight:FontWeight.bold,
//                           ),
//                           ),
//                         ],
//                       ),
//                     ),
//                   ),
//
//       ],
//               ),
//             ),
//             Divider(thickness:2,color:Colors.blueGrey,),
//             Row(
//             children:[
//               Container(
//                 height:25,
//                 width:30,
//                 decoration:BoxDecoration(
//                   borderRadius:BorderRadius.circular(10),
//                     image:DecorationImage(
//                   image:AssetImage('assets/hijab.jpg'),
//               fit:BoxFit.cover,
//                 ),
//
//               ),
//               ),
//               SizedBox(width:5),
//               Column(
//                 children:[
//                   SizedBox(width:10),
//                   Row(
//                     children:[
//                       Text(
//                         'Larkiyo ki baatain',style:TextStyle(
//                         color:Colors.black,fontWeight:FontWeight.bold,fontSize:15,
//                       ),
//                       ),
//                       SizedBox(width:3),
//                       Text(
//                         'Follow',style:TextStyle(
//                         color:Colors.blue,
//                         fontSize:15,
//                       ),
//                       ),
//                     ],
//                   ),
//                   Row(
//                     children:[
//                       Text(
//                         'faiza ahmed',style:TextStyle(
//                         color:Colors.black,fontWeight:FontWeight.bold,
//                         fontSize:10
//                       ),
//                       ),
//                       SizedBox(width:5),
//                       Text(
//                         '3d',style:TextStyle(
//                         color:Colors.black,
//                         fontSize:8,
//                       ),
//                       ),
//                       SizedBox(width:20),
//                       Icon(Icons.change_circle_outlined,color:Colors.blue,),
//                     ],
//                   ),
// ],
//               ),
//               Spacer(),
//               Row(
//               children:[
//               Icon(Icons.more_horiz,),
//               SizedBox(width:50),
//               Icon(Icons.cancel,),
//                 SizedBox(width:100),
//         ],
//               ),
//           ],
//         ),
//             Divider(thickness:2,color:Colors.grey),
//             SizedBox(height:10),
//             Row(
//               children:[
//                 SizedBox(width:200),
//                 Column(
//                   crossAxisAlignment:CrossAxisAlignment.center,
//                   children:[
//                     Container(
//                       height:300,
//                       width:1000,
//                       decoration:BoxDecoration(
//                         borderRadius:BorderRadius.circular(50),
//                         border:Border.all(color:Colors.black,width:20),
//                         image:DecorationImage(
//                           image:AssetImage('assets/girl3.jpg'),
//                           fit:BoxFit.cover,
//                         ),
//                       ),
//                     ),
//                     SizedBox(width:200),
//         ],
//                 ),
//               ],
//             ),
//             Divider(thickness:2,color:Colors.grey),
//             SizedBox(height:10),
//             Row(
//               mainAxisAlignment: MainAxisAlignment.spaceEvenly,
//               children:[
//                 Row(
//                   children:[
//                     Icon(Icons.thumb_up_alt),
//                     Text(
//                       'like',
//                     ),
//                   ],
//                 ),
//                 Row(
//                   children:[
//                     Icon(Icons.comment_bank),
//                     Text(
//                         'comment'
//                     ),
//                   ],
//                 ),
//                 Row(
//                   children:[
//                     Icon(Icons.send),
//                     Text(
//                       'send',
//                     ),
//                   ],
//                 ),
//                 Row(
//                   children:[
//                     Icon(Icons.share),
//                     Text(
//                       'share',
//                     ),
//                   ],
//                 ),
//               ] ,
//             ),
//
//         ],
//       ),
//     ),
//       ),
//     );
//   }
// }
// import 'package:flutter/material.dart';
// void main()
// {
//   runApp(MyApp());
// }
// class MyApp extends StatelessWidget{
//   const MyApp();
//   @override
//   Widget build(BuildContext context){
//     return MaterialApp(
//       debugShowCheckedModeBanner:false,
//       home:Scaffold(
//         backgroundColor:Colors.white,
//         appBar:
//           AppBar(
//             backgroundColor: Colors.white,
//           actions:[
//            SizedBox(width:500),
//             Icon(Icons.add,color:Colors.black,),
//             SizedBox(width:50),
//             Icon(Icons.more_vert,color:Colors.black,),
//             SizedBox(width:20),
//           ],
//           title:Text(
//             'Status',style:TextStyle(
//             color:Colors.black,
//             fontWeight:FontWeight.bold,fontSize:30,
//           ),
//           ),
//             centerTitle:false,
//       ),
//    body:SingleChildScrollView(
//     child:Column(
//       children: [
//         Row
//           (
//                children:
//                [
//                  Row
//                    (
//                    children: [
//                      CircleAvatar(
//                        radius:40,
//                        backgroundImage:AssetImage(
//                          'assets/awesome.jpg',
//                        ),
//                        child:Column(
//                          crossAxisAlignment: CrossAxisAlignment.center,
//                          children: [
//                            Icon(Icons.add_circle,color:Colors.green),
//                          ],
//                        ),
//                      ),
//                      SizedBox(width:15),
//                      Column(
//                        children:[
//                        Row(
//                            children:[
//                            Text(
//                              'My Status',style:TextStyle(
//                              color:Colors.black,fontWeight:FontWeight.bold,
//                              fontSize:20,
//                            ),
//                            ),
//    ],
//    ),
//
//                              SizedBox(height:5),
//                              Row
//                                (
//                                children:[
//                                  SizedBox(width:30),
//                              Text(
//                                'click to add status update',
//                                style:TextStyle(
//                                  color:Colors.blueGrey,
//                                  fontSize:10,
//                                )
//                              ),
//          ],
//                              ),
// ],
//                        ),
//                          ],
//                  ),
//
//                        ],
//                  ),
//
//         Divider(thickness:5,color:Colors.grey,),
//         Row(
//           children:[
//             SizedBox(width:50),
//             Text(
//               'RECENT',style:TextStyle(
//               color:Colors.green,fontSize:30,fontWeight:FontWeight.bold,
//             ),
//             )
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//          Row(
//            children:[
//            CircleAvatar(
//              radius:20,
//              backgroundImage:AssetImage('assets/beutifull.jpg'),
//            ),
//       ],
//          ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Shazii',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 4:34',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/girl2.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Farzana Atif',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 5:03',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/girl5.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Sabuu',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 5:47',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//      ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/hijab.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'faria',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 7:34',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/girl8.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Sana',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 7:38',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/hq720.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Rukhsar',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 7:44',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/beutifull.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Shazii',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 4:34',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/girl.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Farisa',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 8:34',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//         Divider(thickness:0.5,color:Colors.grey,),
//         Row(
//           children:[
//             Row(
//               children:[
//                 CircleAvatar(
//                   radius:20,
//                   backgroundImage:AssetImage('assets/boy2.jpg'),
//                 ),
//               ],
//             ),
//             Column(
//               children:[
//                 Row(
//                   children:[
//                     SizedBox(width:10),
//                     Text(
//                       'Maniii',style:TextStyle(
//                       color:Colors.black,fontWeight:FontWeight.bold,
//                       fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//                 SizedBox(height:5),
//                 Row(
//                   children:[
//                     SizedBox(width:40),
//                     Text(
//                       'today at 9:34',style:TextStyle(
//                       color:Colors.grey,fontSize:20,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//           ],
//         ),
//       ],
//     ),
//
//       ),
//
//     ),
//     );
//   }
// // }
// import 'package:flutter/cupertino.dart';
//
// import 'package:flutter/material.dart';
// void main()
// {
//   runApp(MyApp());
// }
// class MyApp extends StatelessWidget{
//   const MyApp();
//   @override
//   Widget build(BuildContext context){
//     return MaterialApp(
//       debugShowCheckedModeBanner:false,
//       home:Scaffold(
//         backgroundColor: Colors.white,
//       appBar:AppBar(
//         backgroundColor: Colors.redAccent,
//         actions:[
//           Spacer(),
//           Icon(Icons.search,color:Colors.black,),
//           SizedBox(width:10),
//           Icon(Icons.more_vert,color:Colors.black,),
//         ],
//         title:Text(
//           'Updates',style:TextStyle(
//           color:Colors.purpleAccent,fontSize:30,fontWeight:FontWeight.bold,),
//         ),
//         centerTitle:false,
//       ),
//       body:SingleChildScrollView(
//     child:Column(
//                     children: [
//                       Row(
//                         children:[
//                           SizedBox(width:10),
//                           Container(
//                             height:40,
//                             width:500,
//                             decoration:BoxDecoration(
//                               color:Colors.grey,
//                               borderRadius:BorderRadius.circular(10),
//                               border:Border.all(
//                                 width:2,
//                                 color:Colors.black,
//                               ),
//                             ),
//                             child:Row(
//                               children:[
//                                 Row(
//                                   children:[
//                                    Icon(Icons.search,color:Colors.black,size:20,),
//                                   ],
//                                 ),
//                                 Row(
//                                   children:[
//                                     SizedBox(width:10),
//                                       Text('search',style:TextStyle(
//                                         color:Colors.black,fontWeight:FontWeight.bold,
//                                         fontSize:15,
//                                       )),
//                                   ],
//                                 ),
//                               ],
//                             ),
//                           ),
//                         ],
//                       ),
//                       Divider(thickness:0.5,color:Colors.grey),
//                           Row(
//                         children:[
//                           SizedBox(width:5),
//                         Text(
//                           'Status',style:TextStyle(
//                           color:Colors.black,fontSize:30,
//                           fontWeight:FontWeight.bold,
//                         ),
//                         ),
//                         ],
//                       ),
//                         Divider(thickness:0.5,color:Colors.grey),
//                         Row(
//                           children:[
//                             Container(
//                               height:240,
//                               width:160,
//                               decoration:BoxDecoration(
//                                 borderRadius: BorderRadius.circular(10),
//                              color:Colors.blueGrey,
//                             ),
//                               child:Padding(
//                                 padding:EdgeInsets.all(10.0),
//                                 child:Column(
//                                   crossAxisAlignment: CrossAxisAlignment.center,
//                                   children:[
//                                     CircleAvatar(
//                                       radius:20,
//                                       backgroundImage:AssetImage('assets/awesome.jpg',),
//                                     ),
//                                     SizedBox(height:150),
//                                     Text(
//                                       'Add status',style:TextStyle(
//                                       color:Colors.black,fontWeight:FontWeight.bold,
//                                     ),
//                                     ),
//                                   ],
//                                 ),
//                               ),
//                             ),
//                             SizedBox(width:10),
//                             Container(
//                               height:240,
//                               width:160,
//                               decoration:BoxDecoration(
//                                 borderRadius:BorderRadius.circular(10),
//                                 image:DecorationImage(
//                                   image:AssetImage('assets/bday.jpg'),
//                                 fit:BoxFit.cover,
//                                 ),
//                               ),
//                               child:Padding(
//                                 padding:EdgeInsets.all(10.0),
//                                 child:Column(
//                                   crossAxisAlignment: CrossAxisAlignment.center,
//                                   children:[
//                                     CircleAvatar(
//                                       radius:20,
//                                   backgroundImage:AssetImage('assets/girl2.jpg',),
//                                     ),
//                                     SizedBox(height:150),
//                                     Text(
//                                       'Farisa',style:TextStyle(
//                                        color:Colors.black,fontWeight:FontWeight.bold, ),
//                                     ),
//                                   ],
//                                 ),
//                               ),
//                               ),
//                             SizedBox(width:10),
//                             Container(
//                               height:240,
//                               width:160,
//                               decoration:BoxDecoration(
//                                 borderRadius:BorderRadius.circular(10),
//                                 image:DecorationImage(
//                                   image:AssetImage('assets/girl8.jpg'),
//                                   fit:BoxFit.cover,
//                                 ),
//                               ),
//                               child:Padding(
//                                 padding:EdgeInsets.all(10.0),
//                                 child:Column(
//                                   crossAxisAlignment: CrossAxisAlignment.center,
//                                   children:[
//                                     CircleAvatar(
//                                       radius:20,
//                                       backgroundImage:AssetImage('assets/girl5.jpg',),
//                                     ),
//                                     SizedBox(height:150),
//                                     Text(
//                                       'Ayesha',style:TextStyle(
//                                       color:Colors.black,fontWeight:FontWeight.bold, ),
//                                     ),
//                                   ],
//                                 ),
//                               ),
//                             ),
//                             SizedBox(width:10),
//                             Container(
//                               height:240,
//                               width:160,
//                               decoration:BoxDecoration(
//                                 borderRadius:BorderRadius.circular(10),
//                                 image:DecorationImage(
//                                   image:AssetImage('assets/beach.jpg'),
//                                   fit:BoxFit.cover,
//                                 ),
//                               ),
//                               child:Padding(
//                                 padding:EdgeInsets.all(10.0),
//                                 child:Column(
//                                   crossAxisAlignment: CrossAxisAlignment.center,
//                                   children:[
//                                     CircleAvatar(
//                                       radius:20,
//                                       backgroundImage:AssetImage('assets/girl.jpg',),
//                                     ),
//                                     SizedBox(height:150),
//                                     Text(
//                                       'shaziii',style:TextStyle(
//                                       color:Colors.black,fontWeight:FontWeight.bold, ),
//                                     ),
//                                   ],
//                                 ),
//                               ),
//                             ),
//                             SizedBox(width:10),
//                             Container(
//                               height:240,
//                               width:160,
//                               decoration:BoxDecoration(
//                                 borderRadius:BorderRadius.circular(10),
//                                 image:DecorationImage(
//                                   image:AssetImage('assets/party.jpg'),
//                                   fit:BoxFit.cover,
//                                 ),
//                               ),
//                               child:Padding(
//                                 padding:EdgeInsets.all(10.0),
//                                 child:Column(
//                                   crossAxisAlignment: CrossAxisAlignment.center,
//                                   children:[
//                                     CircleAvatar(
//                                       radius:20,
//                                       backgroundImage:AssetImage('assets/girl5.jpg',),
//                                     ),
//                                     SizedBox(height:150),
//                                     Text(
//                                       'Maryam',style:TextStyle(
//                                       color:Colors.black,fontWeight:FontWeight.bold, ),
//                                     ),
//                                   ],
//                                 ),
//                               ),
//                             ),
//                             SizedBox(width:10),
//                             Container(
//                               height:240,
//                               width:160,
//                               decoration:BoxDecoration(
//                                 borderRadius:BorderRadius.circular(10),
//                                 image:DecorationImage(
//                                   image:AssetImage('assets/boy2.jpg'),
//                                   fit:BoxFit.cover,
//                                 ),
//                               ),
//                               child:Padding(
//                                 padding:EdgeInsets.all(10.0),
//                                 child:Column(
//                                   crossAxisAlignment: CrossAxisAlignment.center,
//                                   children:[
//                                     CircleAvatar(
//                                       radius:20,
//                                       backgroundImage:AssetImage('assets/boy.jpg',),
//                                     ),
//                                     SizedBox(height:150),
//                                     Text(
//                                       'usman',style:TextStyle(
//                                       color:Colors.black,fontWeight:FontWeight.bold, ),
//                                     ),
//                                   ],
//                                 ),
//                               ),
//                             ),
//                             SizedBox(width:10),
//                             Container(
//                               height:240,
//                               width:160,
//                               decoration:BoxDecoration(
//                                 borderRadius:BorderRadius.circular(10),
//                                 image:DecorationImage(
//                                   image:AssetImage('assets/hijab.jpg'),
//                                   fit:BoxFit.cover,
//                                 ),
//                               ),
//                               child:Padding(
//                                 padding:EdgeInsets.all(10.0),
//                                 child:Column(
//                                   crossAxisAlignment: CrossAxisAlignment.center,
//                                   children:[
//                                     CircleAvatar(
//                                       radius:20,
//                                       backgroundImage:AssetImage('assets/girl.jpg',),
//                                     ),
//                                     SizedBox(height:150),
//                                     Text(
//                                       'faria',style:TextStyle(
//                                       color:Colors.black,fontWeight:FontWeight.bold, ),
//                                     ),
//                                   ],
//                                 ),
//                               ),
//                             ),
//                           ],
//                         ),
//                         Row(
//                           children:[
//                             Row(
//                               children:[
//                               SizedBox(width:10),
//                               Text(
//                                 'Channels',style:TextStyle(
//                                 color:Colors.black,fontSize:30,fontWeight:FontWeight.bold,
//                               ),
//                               ),
//       ],
//                             ),
//                           ],
//                         ),
//                         Card(
//                           color:Colors.grey,
//                         child: ListTile(
//                          leading:CircleAvatar(
//                            radius:40,
//                            backgroundImage:AssetImage('assets/hq720.jpg'),
//       ),
//                            title:Text(
//                              'Food Fusion',
//                            ),
//                           subtitle:
//                           Row(
//                             children:[
//                               Icon(Icons.video_camera_front_sharp,),
//                             SizedBox(width:3),
//                             Text(
//                           'Roast Masala chicken recipe',
//                           ),
//                           ],
//                           ),
//                           trailing:Column(
//                             children:[
//                               Text(
//                                 '8:03pm',style:TextStyle(
//                             color:Colors.black,
//                           ),
//                               ),
//                               CircleAvatar(
//                                 radius:15,
//                                 backgroundColor:Colors.red,
//                                 child:Text(
//                                   '8',
//                                 ),
//                               ),
//                             ],
//                           ),
//
//     ),
//                       ),
//                       Card(
//                         color:Colors.grey,
//                         child:ListTile(
//                           leading:CircleAvatar(
//                             radius:40,
//                             backgroundImage:AssetImage(
//                               'assets/hijab.jpg',
//                             ),
//                           ),
//                           title:Text(
//                             'Graphic Designing',
//                           ),
//                           subtitle:Row(
//                                 children: [
//                                   Icon(Icons.add_a_photo_rounded),
//                                   SizedBox(width:3),
//                                   Text(
//                                     'this is an idea to....',
//                                   ),
//                                 ],
//                           ),
//                             trailing:Column(
//                           children:[
//                             Text(
//                               '7:30',style:TextStyle(
//                                color:Colors.black,
//                             ),),
//                             CircleAvatar(
//                               radius:20,
//                               backgroundColor: Colors.red,
//                               child:Text(
//                                 '3',
//                               ),
//                             ),
//                             ],
//                         ),
//                           ),
//                         ),
//                       Card(
//                         color:Colors.grey,
//                         child:ListTile(
//                           leading:CircleAvatar(
//                             radius:40,
//                             backgroundImage:AssetImage(
//                               'assets/boy7.jpg',
//                             ),
//                           ),
//                           title:Text(
//                             'Marketing Textile',
//                           ),
//                           subtitle:Row(
//                             children: [
//                               Icon(Icons.add_a_photo_rounded),
//                               SizedBox(width:3),
//                               Text(
//                                 'best latest digital marketing........',
//                               ),
//                             ],
//                           ),
//                           trailing:Column(
//                             children:[
//                               Text(
//                                 '7:20',style:TextStyle(
//                                 color:Colors.black,
//                               ),),
//                               CircleAvatar(
//                                 radius:20,
//                                 backgroundColor: Colors.red,
//                                 child:Text(
//                                   '2',
//                                 ),
//                               ),
//                             ],
//                           ),
//                         ),
//                       ),
//                     ],
//                   ),
//               ),
//
//       ),
//
//     );
//   }
// }
// import 'package:flutter/cupertino.dart';
// import 'package:flutter/material.dart';
// void main()
// {
//   runApp(MyApp());
// }
// class MyApp extends StatelessWidget{
//   const MyApp();
//   @override
//   Widget build(BuildContext context){
//     return MaterialApp(
//       debugShowCheckedModeBanner:false,
//       home:Scaffold(
//         backgroundColor: Colors.black,
//         appBar:AppBar(
//           backgroundColor: Colors.blueGrey,
//           actions:[
//             Icon(Icons.search,color:Colors.black,),
//             SizedBox(width:15),
//             Icon(Icons.more_vert,color:Colors.black,),
//           ],
//           title:Text(
//             'WhatsApp',style:TextStyle(
//             color:Colors.grey,
//             fontSize:30,
//             fontWeight:FontWeight.bold,
//           ),
//           ),
//           centerTitle:false,
//           ),
//         body:Column(
//           children:[
//             Row(
//               mainAxisAlignment:MainAxisAlignment.spaceEvenly,
//          children:[
//              Icon(Icons.camera_alt,color:Colors.white54,size:30,),
//            Text(
//              'CHATS',style:TextStyle(
//              color:Colors.white54,fontSize:30,
//            ),
//            ),
//            Text(
//              'STATUS',style:TextStyle(
//              color:Colors.white54,fontSize:30,
//            ),
//            ),
//            Text(
//              'CALLS',style:TextStyle(
//              color:Colors.white54,fontSize:30,
//            ),
//            ),
//
//          ],
//             ),
//             Divider(thickness:0.5,color:Colors.blueGrey,),
//             Row(
//               children:[
//                 SizedBox(width:20),
//                 CircleAvatar(
//                   radius:30,
//                   backgroundImage:AssetImage('assets/girl8.jpg'),
//                   child:Column(
//                     crossAxisAlignment:CrossAxisAlignment.end,
//                     children:[
//                       Icon(Icons.add_circle,color:Colors.blue,size:8),
//                     ],
//                   ),
//                 ),
//                 SizedBox(width:15),
//                 Column(
//                   children:[
//                     Row(
//                       children:[
//                         Text(
//                           'My Status',style:TextStyle(
//                           color:Colors.white54,fontSize:18,
//                         ),
//                         ),
//                       ],
//                     ),
//                     Row(
//                       children:[
//                         Text(
//                           'tap to add status',style:TextStyle(
//                           color:Colors.white54,fontSize:10,
//                         ),
//                         ),
//                       ],
//                     ),
//                   ],
//                 ),
//               ],
//             ),
//             Divider(thickness:0.5,color:Colors.blueGrey,),
//             Row(
//              children:[
//                SizedBox(width:20),
//                Text(
//                  'Recent Updates',style:TextStyle(
//                  color:Colors.white54,fontSize:30,fontWeight:FontWeight.bold,
//                ),
//                ),
//              ],
//             ),
//             Divider(thickness:0.5,color:Colors.grey,),
//             SizedBox(height:10),
//             Row(
//               children:[
//               SizedBox(height:10),
//               CircleAvatar(
//                 radius:30,
//                 backgroundColor:Colors.green,
//                 child:CircleAvatar(
//                   radius:25,
//                   backgroundImage:AssetImage('assets/boy5.jpg'),
//                 ),
//               ),
//               SizedBox(width:50),
//               Column(
//                 children:[
//                   Text(
//                     'XYZZZZZZ',style:TextStyle(
//                     color:Colors.white,
//                     fontSize:25,
//                   ),
//                   ),
//                   Text(
//                     '36 minutes ago',style:TextStyle(
//                     color:Colors.blueGrey,
//                     fontSize:10,
//                   ),
//                   ),
//                 ],
//               ),
//         ],
//             ),
//             SizedBox(height:10),
//             Row(
//            children:[
//              SizedBox(height:10),
//              CircleAvatar(
//                radius:30,
//                backgroundColor:Colors.green,
//                child:CircleAvatar(
//                  radius:25,
//                  backgroundImage:AssetImage('assets/boy2.jpg'),
//                ),
//              ),
//              SizedBox(width:50),
//              Column(
//                children:[
//                  Text(
//                 'VIEW Eee appi santoshi',style:TextStyle(
//                    color:Colors.white,
//                    fontSize:25,
//                  ),
//                  ),
//                  Text(
//                    '16 minutes ago',style:TextStyle(
//                    color:Colors.blueGrey,
//                    fontSize:10,
//                  ),
//                  ),
//                ],
//              ),
//            ],
//             ),
//             Row(
//               children:[
//                 SizedBox(height:10),
//               CircleAvatar(
//               radius:30,
//                 backgroundColor:Colors.green,
//                 child:CircleAvatar(
//                   radius:25,
//                   backgroundImage:AssetImage('assets/boy.jpg'),
//                 ),
//               ),
//                 SizedBox(width:50),
//                 Column(
//                   children:[
//                     Text(
//                       'VIEW C Chitanya',style:TextStyle(
//                       color:Colors.white,
//                       fontSize:25,
//                     ),
//                     ),
//                     Text(
//                       '16 minutes ago',style:TextStyle(
//                       color:Colors.blueGrey,
//                       fontSize:10,
//                     ),
//                     ),
//                   ],
//                 ),
//               ],
//
//             ),
//             SizedBox(height:10),
//             Row(
//                      children: [
//                        SizedBox(height:10),
//                        CircleAvatar(
//                          radius:30,
//                          backgroundColor:Colors.green,
//                          child:CircleAvatar(
//                            radius:25,
//                            backgroundImage:AssetImage('assets/boy5.jpg'),
//                          ),
//                        ),
//                        SizedBox(width:50),
//                        Column(
//                          children:[
//                            Text(
//                              'VIEW De monzzzz',style:TextStyle(
//                              color:Colors.white,
//                              fontSize:25,
//                            ),
//                            ),
//                            Text(
//                              '16 minutes ago',style:TextStyle(
//                              color:Colors.blueGrey,
//                              fontSize:10,
//                            ),
//                            ),
//                          ],
//                        ),
//                  ],
//                ),
//             Row(
//               children:[
//                 Spacer(),
//                 Column(
//                   crossAxisAlignment:CrossAxisAlignment.end,
//                   children:[
//                     Icon(Icons.edit,color:Colors.white,),
//                     SizedBox(width:200),
//                   ],
//                 ),
//               ],
//             ),
//             SizedBox(height:15,),
//             Row(
//            children:[
//              Spacer(),
//              Column(
//                crossAxisAlignment:CrossAxisAlignment.end,
//                children:[
//                CircleAvatar(
//                  backgroundColor: Colors.green,
//                  radius:25,
//                  child:Center(
//                 child: Icon(Icons.camera_alt,color:Colors.black,),
//                ) ,
//                ),
//                SizedBox(width:200),
//                ],
//              ),
//            ],
//             ),
//         ],
//         ),
//       ),
//     );
//   }
//}
import 'package:flutter/material.dart';
void main(){
  runApp(MyApp());
}
class MyApp extends StatelessWidget{
  const MyApp();
  @override
  Widget build(BuildContext context){
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      home:Scaffold(
        backgroundColor: Colors.white,
        appBar:AppBar(
          backgroundColor: Colors.white,
          actions:[
            Icon(Icons.camera_alt,color:Colors.black,),
            SizedBox(width:20),
            Icon(Icons.more_vert,color:Colors.black,),
            SizedBox(width:30),
          ],
          title:Text(
            'WhatsApp',style:TextStyle(
            color:Colors.green,
            fontSize:30,
            fontWeight:FontWeight.bold,
          ),
          ),
          centerTitle:false,
        ),
        body:SingleChildScrollView(
          child: Column(
              children:[

                Container(
                  height:80,
                  width:270,
                  decoration:BoxDecoration(
                    borderRadius: BorderRadius.circular(10),
                    color:Colors.white54,

                  ),
                  child:Row(
                   children:[
                     Icon(Icons.search,color:Colors.black),
                     SizedBox(width:15),
                     Text(
                       'Search',style:TextStyle(
                       color:Colors.black,fontSize:20,fontWeight:FontWeight.bold,
                     ),
                     ),
                   ],
                  ),
                ),
                Divider(thickness:0.5,color:Colors.blueGrey),
                Row(
                 children:[
                   Container(
                    height:25,
                     width:100,
                     decoration: BoxDecoration(
                       color:Colors.white54,
                       borderRadius:BorderRadius.circular(10),
                     ),
                     child:Text(
                       'All',style:TextStyle(
                       color:Colors.black,
                     ),
                     ),
                   ),
                   Container(
                     height:25,
                     width:85,
                     decoration: BoxDecoration(
                       color:Colors.white54,
                       borderRadius:BorderRadius.circular(10),
                     ),
                     child:Text(
                       'UnRead Chats',style:TextStyle(
                       color:Colors.black,
                     ),
                     ),
                   ),

                    Container(
                      height:40,
                      width:100,
                      decoration: BoxDecoration(
                        color:Colors.white54,
                        borderRadius:BorderRadius.circular(10),
                      ),
                      child:Text(
                        'Favourites',style:TextStyle(
                        color:Colors.black,
                      ),
                      ),
                    ),
                   Container(
                     height:20,
                     width:75,
                     decoration: BoxDecoration(
                       color:Colors.white54,
                       borderRadius:BorderRadius.circular(10),
                     ),
                     child:Text(
                       'Groups',style:TextStyle(
                       color:Colors.black,
                     ),
                     ),
                   ),
                   Container(
                     height:35,
                     width:75,
                     decoration: BoxDecoration(
                       color:Colors.white54,
                       borderRadius:BorderRadius.circular(10),
                     ),
                     child:Icon(Icons.add,color:Colors.black,)

                   ),
      ],
                   ),
               Divider(thickness:0.5,color:Colors.blueGrey,),
                Card(
                  color:Colors.white,
                  child:ListTile
                    (
                    leading:CircleAvatar(
                      radius:40,
                      backgroundImage: AssetImage('assets/awesome.jpg'),
                    ),
                    title:Text(
                      'Fatima',style:TextStyle(
                      color:Colors.black,
                      fontSize:20,
                    ),
                    ),
                    subtitle: Text(
                      'wht about today??',
                      style:TextStyle(
                        color:Colors.black,
                        fontSize:10,
                      ),
                    ),
                    trailing:Column(
                      children:[
                        Text(
                          '6:03am',
                        ),
                        SizedBox(height:5),
                        CircleAvatar(
                        radius:20,
                          backgroundColor: Colors.red,
                          child:Text(
                            '2',
                          ),
                        ),
                      ],
                    ),
                  ),
                ),
                Card(
                  color:Colors.white,
                  child:ListTile
                    (
                    leading:CircleAvatar(
                          radius:40,
                          backgroundImage: AssetImage('assets/beutifull.jpg.jpg'),
                    ),
                    title:Text(
                          'shazii',style:TextStyle(
                          color:Colors.black,
                          fontSize:20,
                    ),
                    ),
                    subtitle: Text(
                          'are u here?',
                          style:TextStyle(
                            color:Colors.black,
                            fontSize:10,
                          ),
                    ),
                    trailing:Column(
                          children:[
                            Text(
                              '6:10am',
                            ),
                            SizedBox(height:5),
                            CircleAvatar(
                              radius:20,
                              backgroundColor: Colors.red,
                              child:Text(
                                '5',
                              ),
                            ),
                          ],
                    ),
                  ),
                ),
                Card(
                  color:Colors.white,
                  child:ListTile
                    (
                    leading:CircleAvatar(
                          radius:30,
                          backgroundImage: AssetImage('assets/awesome.jpg'),
                    ),
                    title:Text(
                          'Fatima',style:TextStyle(
                          color:Colors.black,
                          fontSize:20,
                    ),
                    ),
                    subtitle: Text(
                          'do you have some plain today??',
                          style:TextStyle(
                            color:Colors.black,
                            fontSize:10,
                          ),
                    ),
                    trailing:Column(
                          children:[
                            Text(
                              '8:30am',style:TextStyle(
                        color:Colors.blueGrey,
                    ),
                            ),
                            SizedBox(height:5),
                            CircleAvatar(
                              radius:20,
                              backgroundColor: Colors.red,
                              child:Text(
                                '5',style:TextStyle(
                                color:Colors.black,fontSize:10,
                              ),
                              ),
                            ),
                          ],
                    ),
                  ),
                ),
              ],
            ),
        ),
      ),

    );
  }
}